...
// each bar has the following properties
class Element {
    constructor(val) {
        this.val = val;
        this.compare = false;
    ...
    draw(i, color = [255, 255, 255, 0]) { // Default color with transparency
        fill(color);
        if (this.compare) {
            fill(168, 216, 234); // Soft Blue (#A8D8EA)
        }
        if (this.swap) {
            fill(250, 218, 221); // Light Pink (#FADADD)
        }
        if (sorted_arr[i] == arr[i].val) {
            fill(180, 225, 184); // Mint Green (#B4E1B8)
        }
        stroke(0);
        this.swap = false;
        ...
        // Commenting out ellipse drawing to maintain sharp edges
        // if (thick > 5) {
...
function setup() {
    ...
    // Adjust the canvas to be at the bottom
...
function draw() {
    clear(); // This makes the background of the canvas transparent
...
function draw_arr() {
    for (let i = 0; i < arr.length; i++) {
        arr[i].draw(i);
        if (sorted_arr[i] == arr[i].val) {
            arr[i].draw(i, color(0, 255, 0, 255)); // Fully opaque green
        }
    }
}
